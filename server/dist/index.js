...
var rawConfig = LoadResourceFile(GetCurrentResourceName(), "config.json");
if (!rawConfig)
  throw new Error("Failed to load config.json");
var config = JSON.parse(rawConfig);
var requestDelay = Math.round(60 / config.requestsPerMinute * 1e3);
var allowedExtensionLookup = {
  ...config.extensions.image.reduce((acc, ext) => ({ ...acc, [ext]: "image" }), {}),
  ...config.extensions.video.reduce((acc, ext) => ({ ...acc, [ext]: "video" }), {})
};
var fetchImgurLinksFromDatabase = async (schemaRows) => {
  ...
  for (const row of schemaRows) {
    ...
    if (tableName == "`fivemanage_convert_lookup`")
      continue;
    const imageRows = await import_oxmysql.oxmysql.query(
    ...
    for (const imageRow of imageRows) {
      ...
      if (!image)
        continue;
      if (!occurances[image])
        occurances[image] = { locations: [] };
      if (occurances[image].locations.find((loc) => loc.table == tableName && loc.column == columnName))
        continue;
      ...
      count++;
      inserted[image] = true;
      await import_oxmysql.oxmysql.insert("INSERT IGNORE INTO fivemanage_convert_lookup (og_link) VALUES (?)", [image]);
    }
  }
  for (const [link, { locations }] of Object.entries(occurances)) {
    await import_oxmysql.oxmysql.update("UPDATE fivemanage_convert_lookup SET occurances = ? WHERE og_link = ?", [JSON.stringify(locations), link]);
  }
  inserted = {};
  console.log(`^5[INFO]^7: Fetched ${count} imgur links from the database.`);
  SetResourceKvpInt("fetched", 1);
};
var replaceImagesInDatabase = async (link, newLink, occurances) => {
  for (const occurance of occurances) {
    const { table, column } = occurance;
    if (table != "`fivemanage_convert_lookup`")
      await import_oxmysql.oxmysql.update(`UPDATE ${table} SET ${column} = REPLACE(${column}, ?, ?)`, [link, newLink]);
  }
};
import_oxmysql.oxmysql.ready(async () => {
  const dbName = await import_oxmysql.oxmysql.scalar("SELECT DATABASE()");
  if (!dbName) {
    return console.log("^1[ERROR]^7: Failed to get current database");
  }
  console.log("Using database: ", dbName);
  const hasFetched = GetResourceKvpInt("fetched") == 1;
  const schemaRows = await import_oxmysql.oxmysql.query("SELECT TABLE_NAME, COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ?", [
    dbName
  ]);
  if (!hasFetched) {
    await fetchImgurLinksFromDatabase(schemaRows);
  }
  const totalCount = await import_oxmysql.oxmysql.scalar("SELECT COUNT(1) FROM fivemanage_convert_lookup");
  const countNotConverted = await import_oxmysql.oxmysql.scalar("SELECT COUNT(1) FROM fivemanage_convert_lookup WHERE new_link IS NULL");
  ...
  let failedUploads = 0;
  while (true) {
    const rows = await import_oxmysql.oxmysql.query("SELECT og_link, occurances FROM fivemanage_convert_lookup WHERE new_link IS NULL LIMIT ?, ?", [
      offset,
      limit
    ]);
    for (const row of rows) {
      const link = row.og_link;
      const occurances = row.occurances ? JSON.parse(row.occurances) : [];
      const fullName = link.split("/").pop();
      ...
      try {
        ...
        const formData = new FormData();
        const file = new file_default([Buffer.from(buffer)], `${fileName}.${extension}`, { type: res.headers.get("content-type") });
        ...
        const newLink = (await uploadRes.json()).url;
        if (!newLink)
          throw new Error("Failed to upload image (no url from Fivemanage)");
        await import_oxmysql.oxmysql.update("UPDATE fivemanage_convert_lookup SET new_link = ? WHERE og_link = ?", [newLink, link]);
        await replaceImagesInDatabase(link, newLink, occurances);
        console.log(`^5[INFO]^7: Replaced ${link} with ${newLink}`);
      } catch (e2) {
        console.log(`^1[ERROR]^7: Failed to upload ${link}, error:`, e2);
        failedUploads += 1;
      }
